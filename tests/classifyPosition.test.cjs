...
test('classifyPositionQuality STRONGER when loss small and normal liquidity', () => {
  ...
  const result = classifyPositionQuality(scenarios, marketNormal);
  assert.equal(result.positionQuality, 'STRONGER');
});
...
test('classifyPositionQuality WEAKER when loss moderate but thin liquidity downgrades', () => {
  ...
  const result = classifyPositionQuality(scenarios, marketThin);
  // MIXED downgraded by thin liquidity => WEAKER
  assert.equal(result.positionQuality, 'WEAKER');
});
