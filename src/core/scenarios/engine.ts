...
export function runStressScenarios(trade: NormalizedTrade, state: MarketState, config: ScenarioConfig): StressScenario[] {
  ...
  if (!isTokenizedEquity) {
    ...
    // For directional stress, basis shift must be adverse to the proposed position:
    // LONG suffers when token basis discounts/contracts relative to reference (- points)
    // SHORT suffers when token basis expands/rises relative to reference (+ points)
    const basisShift = trade.direction === "LONG"
      ? -Math.abs(config.basisWideningPctPoints)
      : Math.abs(config.basisWideningPctPoints);
  ...
  const combined = baseScenario(
    ...
    [
      ...
      `Basis widening: ${trade.direction === "LONG" ? "-" : "+"}${config.basisWideningPctPoints} percentage points (adverse directional shift).`,
