...
test("Verdict Scoring - scorePosition band tests with hand-computed expectations", async (t) => {
  await t.test("returns 'clear' band for low-risk well-hedged position (score = 0.85)", () => {
    const signals: PositionSignals = {
      expectedShortfall: 0.05,     // -min(0.05/0.20, 1)*0.40 = -0.10
      valueAtRisk: 0.03,
      positionFraction: 0.05,      // -min(0.05/0.25, 1)*0.25 = -0.05
  ...
  await t.test("returns 'moderate' band for moderate risk position (score = 0.70)", () => {
    const signals: PositionSignals = {
      expectedShortfall: 0.10,     // -min(0.10/0.20, 1)*0.40 = -0.20
      valueAtRisk: 0.06,
      positionFraction: 0.10,      // -min(0.10/0.25, 1)*0.25 = -0.10
      gapExposureFraction: 0.0,    // -0.00
      hedgeCoverageFraction: 0.0,  // +0.00
    };
    // 1.0 - 0.20 - 0.10 = 0.70
  ...
  await t.test("returns 'elevated' band for higher risk position (score = 0.45)", () => {
    const signals: PositionSignals = {
      expectedShortfall: 0.15,     // -min(0.15/0.20, 1)*0.40 = -0.30
      valueAtRisk: 0.10,
      positionFraction: 0.20,      // -min(0.20/0.25, 1)*0.25 = -0.20
      gapExposureFraction: 0.25,   // -0.25 * 0.20 = -0.05
      hedgeCoverageFraction: 0.0,  // +0.00
    };
    // 1.0 - 0.30 - 0.20 - 0.05 = 0.45
    const result = scorePosition(signals);
    assert.equal(result.score, 0.45);
    assert.equal(result.band, "elevated");
  });
...
test("Verdict Scoring - gateVerdict returns the worse band for all 16 combinations", async (t) => {
  const bands: QualityBand[] = ["critical", "elevated", "moderate", "clear"];
  const rank: Record<QualityBand, number> = {
    critical: 0,
    elevated: 1,
    moderate: 2,
    clear: 3,
...
test("Verdict Scoring - Determinism test (100 iterations with identical input)", () => {
...
test("Verdict Scoring - Clamping test for extreme values", () => {
  const extremeSignals: PositionSignals = {
  ...
  const fullHedgeSignals: PositionSignals = {
