...
export function DecisionArtifactView({
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-16">
      ...
      <div className={`border ${verdictInfo.border} ${verdictInfo.bg} p-6 sm:p-8 shadow-xs space-y-4`}>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--rt-border-subtle)] pb-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h2 className={`text-2xl sm:text-3xl font-mono font-bold tracking-tight mt-2 [text-wrap:balance] ${verdictInfo.text}`}>
              {verdictInfo.title}
            </h2>
            <p className="text-sm font-medium text-[var(--rt-text-primary)] mt-1 [text-wrap:pretty]">
              {verdictInfo.description}
            </p>
          </div>

          <div className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-4 sm:text-right shrink-0 shadow-xs">
            <div className="text-xs font-mono text-[var(--rt-text-muted)]">Proposed trade</div>
            <div className="text-base font-mono font-bold text-[var(--rt-text-primary)] mt-0.5">
              <span className={`mr-1 px-1.5 py-0.5 text-xs font-mono font-bold ${
            ...
            <div className="text-xs text-[var(--rt-text-muted)] mt-0.5 font-mono">
        ...
        {limitations.length > 0 && (
          <div className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-3.5 text-xs text-[var(--rt-text-primary)] flex items-start gap-2 [text-wrap:pretty]">
        ...
        {(artifact.dataSource === 'fixture' || artifact.isFallbackDemo || artifact.marketState?.isSynthetic) && (
          <div id="snapshot-data-fixture-banner" className="border border-[var(--rt-verdict-moderate)] bg-[var(--rt-surface-base)] p-3.5 text-xs text-[var(--rt-text-primary)] flex items-start gap-2 [text-wrap:pretty]">
        ...
        {artifact.evidence.some(e => e.state === "UNAVAILABLE" || e.state === "CURATED_DEMO_FIXTURE") && (
          <div className="border border-[var(--rt-verdict-critical)] bg-[var(--rt-surface-base)] p-3.5 text-xs text-[var(--rt-text-primary)] flex items-start gap-2 [text-wrap:pretty]">
        ...
        {(artifact.isFallbackDemo || artifact.marketState?.isFallbackDemo || limitations.some(l => l.includes("Bitget API unavailable"))) && (
          <div id="demo-fallback-alert" className="border border-[var(--rt-verdict-moderate)] bg-[var(--rt-surface-base)] p-3.5 text-xs text-[var(--rt-text-primary)] flex items-start gap-2 [text-wrap:pretty]">
      ...
      {/* 2. Decisive Reasons */}
      <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-3">
        <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 gap-2.5 pt-1">
          {decision.reasons.map((reason, idx) => (
            <div
              key={idx}
              className="flex items-start gap-3 border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-4"
            >
      ...
      {/* 3. Market State Card */}
      <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {thesis ? (
          <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-4 relative">
            <div className="flex items-center gap-2 border-b border-[var(--rt-border-subtle)] pb-3">
              <div className="flex h-5 w-5 items-center justify-center bg-[var(--rt-surface-void)] text-white text-xs font-mono font-bold">
                4
              </div>
              <h3 className="text-base font-mono font-bold text-[var(--rt-text-primary)] [text-wrap:balance] flex-1">
                Thesis deconstruction
              </h3>
              {thesis.modelInfo && (
                <span className="text-[10px] font-mono text-[var(--rt-text-muted)] bg-[var(--rt-surface-base)] px-2 py-0.5 border border-[var(--rt-border-subtle)]" title="AI Model Identity">
            ...
            <div>
              <span className="text-xs font-mono font-semibold text-[var(--rt-text-muted)] uppercase tracking-wider">Normalized thesis</span>
              <p className="mt-1 text-xs sm:text-sm font-medium text-[var(--rt-text-primary)] bg-[var(--rt-surface-base)] p-3.5 border border-[var(--rt-border-subtle)] [text-wrap:pretty] break-words">
                &ldquo;{thesis.normalizedThesis}&rdquo;
              </p>
            </div>

            <div className="space-y-3">
              <span className="text-xs font-mono font-semibold text-[var(--rt-text-muted)] uppercase tracking-wider">Extracted assumptions</span>
              <div className="space-y-2">
                {thesis.assumptions.map((ass, i) => (
                  <div key={i} className="flex items-start justify-between gap-2 text-xs bg-[var(--rt-surface-base)] p-3 border border-[var(--rt-border-subtle)]">
                    <span className="text-[var(--rt-text-primary)] font-medium [text-wrap:pretty]">{ass.text}</span>
                    <span className={`shrink-0 text-xs font-mono font-bold px-2 py-0.5 border border-[var(--rt-border-subtle)] ${
            ...
            {thesis.supportingEvidenceRefs.length > 0 && (
              <div className="space-y-2 pt-1">
                <span className="text-xs font-mono font-semibold text-[var(--rt-text-muted)] uppercase tracking-wider">Provenance links</span>
                <div className="flex flex-wrap gap-2">
                  {thesis.supportingEvidenceRefs.map(ref => (
                    <span key={ref} className="text-[10px] font-mono text-[var(--rt-verdict-clear)] bg-[var(--rt-surface-base)] border border-[var(--rt-border-subtle)] px-1.5 py-0.5">
                      [PROV: {ref}]
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-2 pt-1">
              <span className="text-xs font-mono font-semibold text-[var(--rt-text-muted)] uppercase tracking-wider">Key dependencies</span>
              <ul className="list-disc list-inside text-xs text-[var(--rt-text-primary)] space-y-1">
          ...
          <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-6 shadow-xs flex flex-col items-center justify-center text-center space-y-2">
            <span className="text-xs font-mono font-bold text-[var(--rt-text-muted)] uppercase tracking-wider">Thesis Deconstruction</span>
            <span className="text-sm font-medium text-[var(--rt-text-muted)]">Unavailable due to system degradation.</span>
          </section>
        )}
        ...
        {challenge ? (
          <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-4 relative">
            ...
            <div>
              <span className="text-xs font-mono font-semibold text-[var(--rt-verdict-critical)] uppercase tracking-wider">Strongest counter thesis</span>
              <p className="mt-1 text-xs sm:text-sm font-medium text-[var(--rt-text-primary)] bg-[var(--rt-surface-base)] p-3.5 border border-[var(--rt-border-subtle)] [text-wrap:pretty] break-words">
            ...
            <div className="space-y-3">
              <span className="text-xs font-mono font-semibold text-[var(--rt-verdict-critical)] uppercase tracking-wider">Vulnerable assumptions attacked</span>
              <div className="space-y-2">
                {challenge.vulnerableAssumptions.map((item, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs bg-[var(--rt-surface-base)] p-3 border border-[var(--rt-border-subtle)] text-[var(--rt-text-primary)] font-medium">
            ...
            {challenge.contradictoryEvidenceRefs.length > 0 && (
              <div className="space-y-2 pt-1">
                <span className="text-xs font-mono font-semibold text-[var(--rt-verdict-critical)] uppercase tracking-wider">Contradictory evidence refs</span>
                <div className="flex flex-wrap gap-2">
                  {challenge.contradictoryEvidenceRefs.map(ref => (
                    <span key={ref} className="text-[10px] font-mono text-[var(--rt-verdict-critical)] bg-[var(--rt-surface-base)] border border-[var(--rt-border-subtle)] px-1.5 py-0.5">
                      [PROV: {ref}]
                    </span>
                  ))}
                </div>
      ...
      <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-[var(--rt-border-subtle)] pb-3">
          ...
          <span className="text-xs font-mono text-[var(--rt-text-muted)]">Zero speculative heuristics • Pure deterministic arithmetic</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {scenarios.filter(s => s.id !== "THESIS_FAILURE").map((sc) => {
            ...
            return (
              <div
                key={sc.id}
                className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-4 flex flex-col h-full"
              >
                <div className="flex-1 mb-4">
                  ...
                  <h4 className="text-sm font-bold text-[var(--rt-text-primary)] mt-2 [text-wrap:balance]">
                    {sc.name}
                  </h4>
                  <p className="text-xs text-[var(--rt-text-muted)] mt-1.5 [text-wrap:pretty]">
                    {sc.description}
                  </p>
                </div>

                <div className="border-t border-[var(--rt-border-subtle)] pt-3 space-y-2">
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs font-mono text-[var(--rt-text-muted)]">Shocked token:</span>
                    <span className="font-mono text-xs font-bold text-[var(--rt-text-primary)]">
                      ${sc.shockedTokenPrice ? sc.shockedTokenPrice.toFixed(2) : "N/A"}
                    </span>
                  </div>

                  <div className="flex justify-between items-baseline">
                    <span className="text-xs font-mono text-[var(--rt-text-muted)]">Position P and L:</span>
                  ...
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs font-mono text-[var(--rt-text-muted)]">Return shock:</span>
                ...
                {sc.assumptions && sc.assumptions.length > 0 && (
                  <div className="mt-3 bg-[var(--rt-surface-raised)] p-2.5 border border-[var(--rt-border-subtle)] text-xs text-[var(--rt-text-muted)] font-mono [text-wrap:pretty] space-y-1">
                    <div className="text-[10px] font-bold text-[var(--rt-text-muted)] uppercase tracking-wider mb-1">Assumptions:</div>
      ...
      {thesisPosition ? (
        <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 sm:p-8 shadow-xs space-y-5 relative">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center bg-[var(--rt-surface-void)] text-white text-xs font-mono font-bold">
                7
              </div>
              <div>
                <span className="text-xs font-mono font-bold uppercase tracking-wider text-[var(--rt-text-muted)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative">
            ...
            <div className={`border p-5 space-y-3 shadow-xs relative bg-[var(--rt-surface-base)] ${
              ...
              <div className="flex items-center justify-between border-b border-[var(--rt-border-subtle)] pb-2">
                <div className="flex items-center gap-2">
                  <div className="p-1.5 bg-[var(--rt-surface-raised)] text-[var(--rt-text-primary)] border border-[var(--rt-border-subtle)]">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                    </svg>
                  </div>
                  <span className="text-sm font-mono font-bold text-[var(--rt-text-primary)] uppercase tracking-wider">
                    Thesis Quality
                  </span>
                </div>
                <span className={`px-3 py-1 text-xs font-mono font-bold uppercase tracking-wider border border-[var(--rt-border-subtle)] ${
                  thesisPosition.thesisQuality === "STRONGER"
                    ? "bg-[var(--rt-surface-raised)] text-[var(--rt-verdict-clear)]"
                    : "bg-[var(--rt-surface-raised)] text-[var(--rt-verdict-moderate)]"
                }`}>
            ...
            <div className={`border p-5 space-y-3 shadow-xs relative bg-[var(--rt-surface-base)] ${
              ...
              {thesisPosition.positionQuality.reasons && thesisPosition.positionQuality.reasons.length > 0 && (
                <div className="mt-2 text-xs font-mono text-[var(--rt-text-muted)] bg-[var(--rt-surface-raised)] p-2 border border-[var(--rt-border-subtle)]">
                  <div className="font-semibold text-[var(--rt-text-primary)] mb-1 uppercase tracking-wide">Deterministic Drivers</div>
          ...
          {thesisPosition.keyMismatch && (
            <div className="border border-[var(--rt-verdict-moderate)] bg-[var(--rt-surface-base)] p-4 space-y-1.5">
        ...
        <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-6 shadow-xs flex flex-col items-center justify-center text-center space-y-2">
          ...
          <span className="text-sm font-medium text-[var(--rt-text-muted)]">Unavailable due to system degradation. Proceeding on deterministic bounds only.</span>
        </section>
      )}
      ...
      <section className="border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-raised)] p-6 shadow-xs space-y-4">
        ...
        <div className="space-y-2">
          {changeConditions.map((condition, idx) => (
            <div
              key={idx}
              className="flex items-start gap-3 border border-[var(--rt-border-subtle)] bg-[var(--rt-surface-base)] p-3.5"
            >
